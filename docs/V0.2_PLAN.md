...
### Rolled forward to v0.2.1

These close real user-visible gaps but are small enough to land as a point release:
...
- **`smtEquiv` overloads for `Z3Array[K, V]` and `Z3DatatypeValue[T]`** (originally v0.1 §18 step 11). Currently workaroundable via `smtValid(a == b)` but the overloads were promised.
