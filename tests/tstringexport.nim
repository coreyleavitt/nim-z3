## v0.5.0 audit fixup #8 — verify z3/string's `export sequence`
## actually surfaces the generic Z3Seq operations on Z3String.
import std/[unittest]
import z3/string
import z3/context  # for newContext

suite "z3/string re-exports z3/sequence":
  test "Z3Seq ops (len, concat, &) are reachable through z3/string":
    let ctx = newContext()
    let s = mkString("abc")
    let t = mkString("xyz")
