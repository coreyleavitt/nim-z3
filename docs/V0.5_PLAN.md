# nim-z3 v0.5 plan (archived)

> **Archived 2026-05-31 after the v0.5.0 tag.** This is the *historical*
> plan that drove v0.5 — the **1.0-readiness polish** release. The live
> plan for ongoing work is
> [IMPLEMENTATION_PLAN.md](IMPLEMENTATION_PLAN.md). Read this file when
> you need design rationale for any of the v0.5 polish work: the
> `z3/context` → `z3/error` extraction; the 13-class typed error
> hierarchy; cross-family parity (generic `pretty[T: Z3Renderable]`,
> generic `astEqual[T: Z3Term]`, `evalXxx` shorthand audit, `$` parity
> via `termToSmt2`); naming + cohesion hygiene (`z3/seq` → `z3/sequence`,
> `RoundingMode` consolidation, `naryOp` macro family, `add`/
> `assertConstraint` resolution); memory + thread safety
> (`tests/tconcurrency.nim`, `nimble valgrind` task, `docs/THREADING.md`);
> Z3 C-API micro-gap closure (`Z3FuncInterp`, `Z3ParamDescrs`,
> `Z3Char↔Z3BitVec[18]`); property tests for v0.3 families; v0.3-family
> examples; user-facing docs (`GOTCHAS.md`, `INTERNAL_API.md`,
> `PARITY.md`, `MINIMAL_BUILD.md`); feature flags; or the v0.5-era §8
> deferral ledger (including the §8b "Pre-tag audit" block); don't
> update it.

A type-safe, memory-safe, idiomatic Nim wrapper for the Z3 SMT solver.

**Status as of archiving**: shipped. v0.4 shipped 2026-05-30 with 1114
OKs; v0.5 followed 2026-05-31 with **1262 OKs** (default config) plus
**18 OKs** for the minimal-build verification across both backends,
zero failures, every §1 goal landed (see §8b). Each step of the
implementation sequence (§5) closed with a §8 deferral entry; the §8b
"Pre-tag audit" block catalogues which v0.5-promised items did and
didn't land before the tag (answer: all 12 §1 goals landed; none
dropped; none rolled to v0.6).

**Audience**: future-me, future contributors, anyone reading this
after v0.5 has shipped and wondering "why is v0.5 the polish release
rather than the feature release?" or reading the rationale for any of
the spec corrections logged in §8.
...
- **`bitvec.mkDistinct` empty-input bug fixed incidentally.** The pre-v0.5 hand-written body indexed `xs[0].ctx` on empty input — would crash at runtime. The unified `emitVarargsDistinctW` helper handles the empty case via `requireCurrentContext()`. Bug → fix → no test added (empty `mkDistinct[W]()` was unreachable in the test suite); flagged here for the audit. *[Superseded by v1.0 audit round 1 phase B5: `emitVarargsDistinctW` was itself collapsed into the single `mkDistinct[T: Z3Term]` generic in `z3/boolean`. The empty-input behaviour carries forward.]*
...
- **Step 7** (property tests): **`tuples3` doesn't exist in proptest** — used nested `tuples2(a, tuples2(b, c))` for 3-arg properties. *[Superseded post-v0.5.0: proptest landed a variadic applicative `map(sa, sb, sc, …)` that replaced both `tuples`/`tuples2` and the `tuples(…).map(unpack)` two-step. Nim-z3 migrated to flat `map(a, b, c)` in the v1.0 audit cycle; consumers receive a clean `(A, B, C)` tuple.]* **FP properties conditioned on `isFinite`** — naïve `x + 0 ≡ x` is false on NaN.
