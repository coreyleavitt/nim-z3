# nim-z3 v0.4 plan (archived)

> **Archived 2026-05-30 after the v0.4.0 tag.** This is the *historical*
> plan that drove v0.4 — the **contract-completion** release. The live
> plan for ongoing work is
> [IMPLEMENTATION_PLAN.md](IMPLEMENTATION_PLAN.md). Read this file when
> you need design rationale for any of the nine v0.4 modules
> (`z3/astvector`, `z3/introspect`, `z3/proof`, `z3/fixedpoint`,
> `z3/rewrite`, `z3/translate`, `z3/probe`, `z3/globalparams`, `z3/io`),
> the solver extensions (`assertConstraintAndTrack` / `getUnsatCore` /
> `getStatistics` / `getConsequences` / `getProof`), the
> `Z3DatatypeValue` runtime registry that unlocks it as a `sortdispatch`
> element, the runtime-erased `Z3AnyAst` family, the quantifier
> introspection extension, the uninterpreted-sort surface added during
> step 14, or the v0.4-era §8 deferral ledger (including the §8b
> "Pre-tag audit" block); don't update it.

A type-safe, memory-safe, idiomatic Nim wrapper for the Z3 SMT solver.

**Status as of archiving**: shipped. v0.3 shipped 2026-05-30 with 890
OKs; v0.4 followed 2026-05-30 with **1114 OKs** across both backends,
zero failures, every §1 goal landed (see §8b). Each step of the
implementation sequence (§5) closed with a §8 deferral entry; the §8b
"Pre-tag audit" block catalogues which v0.4-promised items did and
didn't land before the tag (answer: all 11 §1 goals landed; none
dropped).

**Audience**: future-me, future contributors, anyone deciding whether
v0.4's surface fits their use case or reading the rationale for any of
