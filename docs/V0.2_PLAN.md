# nim-z3 v0.2 plan (archived)

> **Archived 2026-05-29 after the v0.2.0 tag.** This is the *historical*
> plan that drove v0.2; the live plan for ongoing work is
> [IMPLEMENTATION_PLAN.md](IMPLEMENTATION_PLAN.md). Read this file when
> you need design rationale for arrays, datatypes, quantifiers,
> optimization, tactics, or the v0.2-era §8 deferral ledger (including
> the "Pre-tag audit" block near the end of §8); don't update it.

A type-safe, memory-safe, idiomatic Nim wrapper for the Z3 SMT solver.

**Status as of archiving**: shipped. v0.1 had shipped on 2026-05-29; v0.2 followed immediately. Each step of the implementation sequence (§5) closed with a §8 deferral entry; the "Pre-tag audit" block near the end of §8 catalogues which v0.2-promised items did and didn't land before the tag.
...
## Pre-tag audit — items promised "v0.2" that didn't land

Done before the v0.2.0 tag so the rollforward is explicit. Each entry says **where it now goes**.

### Rolled forward to v0.3

**(All Pre-tag audit Category B items rolled into v0.3 — a v0.2.1 point release was considered and rejected since there are no consumers and the work absorbs cleanly into v0.3 step 2.)**

These close real user-visible gaps. Folded into the live `IMPLEMENTATION_PLAN.md` v0.3 plan's §1 goal 2 + §5 step 2:

- **`Z3Model.eval`/`[]` for `Z3DatatypeValue[T]`** (originally promised step 4 §8). Users can't extract datatype values from solver models without it.
- **`Z3Model.eval`/`[]` for `Z3Array[K, V]`** (originally v0.1 §18 step 11, "v0.2 alongside those sort modules"). Same shape gap.
- **`smtEquiv` overloads for `Z3Array[K, V]` and `Z3DatatypeValue[T]`** (originally v0.1 §18 step 11). The v0.3 plan also relocates `smtValid` / `smtEquiv` to a dedicated `z3/semantics` module, closing the discoverability gap surfaced by the post-v0.2 architectural audit.
- **`Z3_apply_result_convert_model`** (originally step 8 §8). Needed for any real use of tactics beyond inspection — model conversion back from subgoals.

### Also rolled forward to v0.3 (longer-horizon items)

- **`evalReal` / `toRealApprox` composer** (originally v0.1 §18 step 8). Needs a clear precision policy before we expose a `float64` extractor for Real values.
- **DOT / GraphViz AST export (`z3/dot`)** (originally v0.1 §18 step 10). Nice-to-have visualisation; not core. Lands when someone has a real debugging use.
- **Wider-width BV recipes (W > 8)** (originally v0.1 §18 step 11). Test-only improvement; recipes need generalisation on W first.
- **Differential testing against `z3` CLI binary** (originally v0.1 §18 step 12). The matrix CI puts `z3` on PATH so the infra is ready; needs a runner abstraction shared with the property suite.
- **valgrind job alongside ASAN** (originally v0.1 §18 step 12). Same class of signal as ASAN, blocked on the same CI blocker as issue #1.
- **`{.optional.}` softlink declarations** (originally v0.1 §18 cross-cutting). Not needed yet — every Z3 symbol we use is in 4.10+. Roll until a v0.3 module needs a newer symbol.
- **Pareto-mode per-point model extraction** for `Z3Optimize` (step 9 §8). Same model-conversion class as the `Z3_apply_result_convert_model` deferral.

### Dropped (won't ship)

- **Colourised pretty output (`prettyColored`)** (originally v0.1 §18 step 10). Original note said "v0.2 if step 13 surfaces demand; drop otherwise" — step 13 didn't surface demand. Dropping.

### Still alive in the §8 ledger above (v0.3 / user-driven trigger), confirmed out of scope for v0.2.0

`Z3_mk_solver_from_tactic`, `Z3_solver_set_params` for `Z3Solver`, arity bumps (constructor `apply` ≥ 6, `declareDatatypes` ≥ 4, `forall`/`exists` ≥ 6), block-DSL macros, nested arrays, `mkArrayExt`, `Z3_optimize_get_objectives`/`get_assertions`/`from_file`, quantifier weight parameter, compile-time "missing patterns" warning, multi-pattern macro.
