...
## generic-proc resolution: any type that has an `==(a, b: T): Z3Bool`
## operator works. v0.2's four families (`Z3Ast[S]`, `Z3BitVec[W]`,
## `Z3Array[K, V]`, `Z3DatatypeValue[T]`) all satisfy that contract,
## as will the v0.3 newcomers (`Z3String`, `Z3Seq[E]`, `Z3Fp[E, S]`,
## `Z3FuncDecl[...]`).
##
...
proc smtEquiv*[T](a, b: T): bool {.inline.} =
  ...
  ## operator — covers `Z3Ast[S]`, `Z3BitVec[W]`, `Z3Array[K, V]`,
  ## `Z3DatatypeValue[T]` (and v0.3's incoming `Z3String` / `Z3Seq[E]`
  ## / `Z3Fp[E, S]` / `Z3FuncDecl[...]`) automatically.
  smtValid(a == b)
